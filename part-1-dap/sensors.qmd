...
## Experimentaufbau {#sec-sensors-experiment-setup .collapsable}
...
## Farbe messen {#sec-sensors-detect-colors .collapsable}
...
Welches Format haben die Werte? Wie Tinkeforge in seiner Online-Dokumentation für den Farbsensor erkläutert, sind die Werte 16-Bit Zahlen, die also von 0 bis 65535 reichen. Ein Wert von 0 bedeutet, dass kein Licht des entsprechenden Farbanteils gemessen wird, während 65535 die maximale Intensität darstellt. Wir haben den RGB-Code bisher mit Werten von 0 bis 255, also als 8-Bit Werte kennengelernt. Da von 8-Bit auf 16-Bit ein Faktor $2^8$ gilt, können wir umgekehrt die 16-Bit Werte in das 8-Bit Format umwandeln, indem wir sie durch 256 teilen. Und zwar Ganzzahlig, damit wir wieder eine ganze Zahl erhalten:
...
## Helligkeit messen {#sec-sensors-measure-light-intensity .collapsable}
...
## Pulse messen {#sec-sensors-measure-pulse .collapsable}
...
## Push vs. Pull {#sec-sensors-push-vs-pull .collapsable}
...
## Morse Code-Empfänger {#sec-sensors-morse-code-receiver .collapsable}

## Pulsmesser {#sec-sensors-heart-rate-monitor .collapsable}

## Dein Finger als Signalquelle

Wir haben oben gesehen, dass der Farbsensor nicht nur die Farbe misst, sondern auch die allgemeine Lichtintensität, auch *Illuminance* genannt. Hier kommt eine spannende Tatsache ins Spiel: Das Prinzip, die Helligkeit zu messen, steckt hinter den Pulsmessern in Fitnesstrackern. Ja, genau der Sensor, der Licht misst, verrät dir, wie schnell dein Herz schlägt! Das Ganze nennt man auch [Photoplethysmografie](https://en.wikipedia.org/wiki/Photoplethysmogram).
...
In der Programmierung lautet ein wichtiges Prinzip: Don't Repeat Yourself! Das gilt insbesondere für Code, den wir selbst geschrieben haben. Doppelte Logiken in unseren Programmen sind eine potenzielle Quelle für Inkonsistenzen und in der Folge Fehler oder unerwartetes Verhalten. Daher ist es gute Praxis, Code in Funktionen auszulagern, wenn wir ihn häufiger verwenden.

Das DRY-Prinzip gilt aber nicht nur auf der Ebene der von uns geschriebenen Funktionen. Immer wenn wir ein Problem lösen wollen sollten wir uns uns zuerst überlegen, ob wir es überhaupt lösen müssen. Oder hat es nicht jemand anderes bereits gelöst und wir können die Lösung einfach verwenden? Dass wir nicht die ersten sind, die mithilfe eines Farbsensors und Python den Puls messen wollen, klingt schließlich naheliegend.

Und so ist es auch: Eine kurze Recherche mit ChatGPT und wir bekommen eine Reihe an Vorschlägen für Bibliotheken, die Funktionen für die Messung von Pulssignalen mitbringen. Darunter auch [NeuroKit2](https://github.com/neuropsychology/NeuroKit), eine Python-Bibliothek, die ihre Autoren selbst als "The Python Toolbox for Neurophysiological Signal Processing" bezeichnen [@Makowski2021neurokit]. Schauen wir uns sie genauer an.

Externe Bibliotheken müssen wir zunächst installieren. Dazu verwenden wir unseren Paketmanager `pip`:

```
pip install neurokit2`
```

Für Mac-User lautet der Befehl entsprechend:
